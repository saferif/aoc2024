...
check maze (r, c) = ((checkDirection maze "MAS" (r - 1, c - 1) (1, 1)) + 
                     (checkDirection maze "MAS" (r - 1, c + 1) (1, -1)) + 
                     (checkDirection maze "MAS" (r + 1, c - 1) (-1, 1)) + 
                     (checkDirection maze "MAS" (r + 1, c + 1) (-1, -1))) > 1

findInRow :: (Array Int (UArray Int Char)) -> (Int, Int) -> Int -> Int
findInRow maze p@(r, c) ans = let row = maze ! r in if inRange (bounds row) c then findInRow maze (r, c + 1) ((fromEnum (check maze p)) + ans) else ans
